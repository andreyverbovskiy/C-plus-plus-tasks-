...
static bool ReadStandbyI2CM(void) {
	uint8_t tc74ConfigRegisterAddress = 1; // 0x01 for config
	uint8_t config[1];
...
int main(void) {
	...
	while(1){
		...
		if(sw3.read()){
			...
			timestamp = (uint32_t)DWT->CYCCNT / 72000;
			uint8_t temperature = ReadTemperatureI2CM();
			print_temp(sample, timestamp, temperature);
			sample++;
		}
		Sleep(1);
		timer++;
...
void set_Standby(){
	...
	uint8_t arr[2] = {tc74ConfigRegisterAddress,STANDBY};
	SetupXferRecAndExecute(
			I2C_TEMP_ADDR,
			arr, 2,
			NULL,0); // empty
...
void set_norm(){
	uint8_t tc74ConfigRegisterAddress = 1;
	uint8_t arr[2] = {tc74ConfigRegisterAddress,0};
	SetupXferRecAndExecute(
			I2C_TEMP_ADDR,
			arr, 2,
			NULL,0); // empty
...
void print_temp(int sample,uint32_t timestamp, uint8_t temperature){
	char buffer[ARRAYSIZE];
	snprintf(buffer,ARRAYSIZE,"\n\r{\"samplenr\": %d,\"timestamp\": %u,\"temperature\":%u\n\r}",sample,timestamp,temperature);
