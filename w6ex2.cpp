...
int main(void) {
	...
	lcd.begin(16, 2);
	lcd.setCursor(0, 0);
	BarGraph bargraph(&lcd,50,false);
	...
	while(1) {
		...
		up = (int)(d0/(float)81.9);

		bargraph.draw(up);

		//Next line
		lcd.setCursor(0,1);
		sprintf(arr2,"%4d",temp);
		lcd.print(arr2);
		lcd.setCursor(4,1); //4th
		low = (int)((float)temp/0.6);
		bargraph.draw(low);
		//Print to ITM console
